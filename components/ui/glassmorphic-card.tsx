...
export const GlassmorphicCard = React.forwardRef<HTMLDivElement, GlassmorphicCardProps>(
  ({ variant = "cyan", holographic = true, className, children, ...props }, ref) => {
    ...
    const getCardStyles = () => {
      ...
      const variantStyles = {
        ...
        gradient: "border border-neon-cyan/30 bg-gradient-to-br from-neon-cyan/5 to-neon-magenta/5",
        subtle: "border border-white/10 bg-white/5",
      }

      return cn(baseStyles, variantStyles[variant], holographic && "holographic", className)
    }
