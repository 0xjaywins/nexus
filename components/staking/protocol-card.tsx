...
export function ProtocolCard({ protocol, onClick }: ProtocolCardProps) {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    cardRef.current.style.transform = `perspective(1000px) rotateX(${rotateX}deg) rotateY(${rotateY}deg) ${isFlipped ? "rotateY(180deg)" : ""}`
  }
  ...
  return (
    ...
    >
      ...
      >
        <h3 className="text-lg font-orbitron text-center neon-cyan mb-2">{protocolData.name}</h3>
        <div className="text-xs text-text-secondary text-center">Click to learn more</div>
      </div>
      ...
      >
        <p className="text-xs text-text-secondary text-center">{protocolData.description}</p>
        <div className="mt-2 text-xs text-neon-cyan text-center">Click to select</div>
      </div>
    </div>
  )
}
